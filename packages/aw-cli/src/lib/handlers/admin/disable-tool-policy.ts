...
/**
 * Disables a tool policy for a specific delegatee in the AW system.
 * This function prompts the user to select a tool and delegatee, then disables the tool policy.
 *
 * @param awAdmin - An instance of the AwAdmin class.
 * @param pkp - The PKP to disable the tool policy for.
 */
export const handleDisableToolPolicy = async (awAdmin: AwAdmin, pkp: PkpInfo) => {
  try {
    ...
    logger.loading('Disabling tool policy...');

    // Disable the tool policy.
    ...
    // Log a success message once the policy is disabled.
    logger.success('Tool policy disabled successfully.');
  } catch (error) {
    // Handle specific errors related to policy disabling.
