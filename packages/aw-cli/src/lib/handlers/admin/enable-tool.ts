...
/**
 * Enables a tool in the AW system.
 * This function prompts the user to select a tool and enables it.
 * Enabling a tool means it can be used right now, separate from policy-level enabling per delegatee.
 *
 * @param awAdmin - An instance of the AwAdmin class.
 * @param pkp - The PKP to enable the tool for.
 */
export const handleEnableTool = async (awAdmin: AwAdmin, pkp: PkpInfo) => {
  try {
    ...
    logger.loading('Enabling tool...');

    try {
      // Enable the tool.
      await awAdmin.enableTool(pkp.info.tokenId, selectedTool.ipfsCid);

      // Log a success message once the tool is enabled.
      logger.success('Tool enabled successfully.');
      logger.info('The tool can now be used by delegatees (subject to their policy settings).');
    } catch (error) {
      if (error instanceof Error) {
        logger.error(`Failed to enable tool: ${error.message}`);
      } else {
        logger.error('Failed to enable tool: Unknown error occurred');
      }
    }
  } catch (error) {
    // Handle specific errors related to tool enabling.
    if (error instanceof AwCliError) {
      ...
      if (error.type === AwCliErrorType.ADMIN_SET_TOOL_POLICY_CANCELLED) {
        ...
        logger.info('Tool enabling cancelled.');
        return;
      }
    }
    ...
    logger.log(`Failed to enable tool: ${errorMessage}`);
  }
}; 