 ...
 */
const batchRemoveDelegatees = async (awAdmin: AwAdmin, addresses: string[]) => {
  // Log a loading message to indicate the operation is in progress.
  logger.loading('Removing delegatees...');

  // Remove the delegatees from the AW system in a batch operation.
  await awAdmin.batchRemoveDelegatees(addresses);
 ...
 * @param awAdmin - An instance of the AwAdmin class.
 */
export const handleBatchRemoveDelegatee = async (awAdmin: AwAdmin) => {
  try {
    ...
    await batchRemoveDelegatees(awAdmin, addresses);
  } catch (error) {
